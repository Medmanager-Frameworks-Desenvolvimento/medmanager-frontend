<template>
  <div class="max-w-4xl mx-auto">
    <h2 class="text-xl font-extrabold text-[#2b4c5e] mb-6">Editar Medicamento</h2>
...
const carregarDadosMedicamento = async () => {
  try {
    const resposta = await MedicamentosService.buscarPorId(medicamentoId);
    const medicamento = resposta.data;

    formulario.nome = medicamento.nome;
    searchNome.value = medicamento.nome; 
    formulario.descricao = medicamento.descricao || '';
    formulario.quantidade = medicamento.quantidade;

    if (medicamento.validade) {
      const dataObj = new Date(medicamento.validade);
      formulario.validade = dataObj.toISOString().split('T')[0];
    }

  } catch (error) {
    console.error("Erro ao carregar medicamento:", error);
    alertType.value = 'error';
    alertMessage.value = 'Não foi possível carregar os dados deste medicamento.';
  }
};
...
const salvarEdicao = async () => {
  ...
  try {
    ...
    await MedicamentosService.atualizar(medicamentoId, {
