<template>
  <div class="max-w-4xl mx-auto">
    <h2 class="text-xl font-extrabold text-[#2b4c5e] mb-6">Editar Paciente</h2>

    <div class="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
      
      <BaseAlert :message="alertMessage" :type="alertType" />

      <form @submit.prevent="salvarEdicao" class="space-y-8">
        ...
        <div>
          ...
          <input 
            ...
            :class="[
              ...
              temErroNoCampo('nome') ? 'border-red-400 focus:ring-red-200' : 'border-gray-300 focus:border-[#2b4c5e] focus:ring-[#2b4c5e]'
            ]"
          />
        </div>
          ...
          <div>
            ...
            <input 
              ...
              :class="[
                ...
                temErroNoCampo('cpf') ? 'border-red-400 focus:ring-red-200' : 'border-gray-300 focus:border-[#2b4c5e] focus:ring-[#2b4c5e]'
              ]"
            />
          </div>
          ...
          <div>
            ...
            <input 
              ...
              :class="[
                ...
                temErroNoCampo('idade') ? 'border-red-400 focus:ring-red-200' : 'border-gray-300 focus:border-[#2b4c5e] focus:ring-[#2b4c5e]'
              ]"
            />
          </div>
        </div>
...
const carregarDadosPaciente = async () => {
  try {
    const resposta = await PacientesService.buscarPorId(pacienteId);
    ...
    const doencasPadrao = ['Diabetes', 'Hipertensão', 'Alzheimer'];
    
    if (paciente.doenca_cronica && paciente.doenca_cronica.length > 0) {
      paciente.doenca_cronica.forEach(doenca => {
        if (doencasPadrao.includes(doenca)) {
          doencasSelecionadas.value.push(doenca);
        } else {
          possuiOutra.value = true;
          outraDoencaTexto.value = doenca;
        }
      });
    }
    ...
    alertMessage.value = 'Não foi possível carregar os dados deste paciente.';
  }
};
...
const salvarEdicao = async () => {
  ...
  try {
    ...
    await PacientesService.atualizar(pacienteId, payload);
