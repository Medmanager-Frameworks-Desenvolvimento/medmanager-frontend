<template>
  <div class="max-w-4xl mx-auto">
    <h2 class="text-xl font-extrabold text-[#2b4c5e] mb-6">Editar Enfermeiro</h2>
...
const enfermeiroId = route.params.id;
...
const carregarDadosEnfermeiro = async () => {
  try {
    const resposta = await EnfermeirosService.buscarPorId(enfermeiroId);
    const enfermeiro = resposta.data;

    form.nome = enfermeiro.nome;
    form.cpf = enfermeiro.cpf;
    form.telefone = enfermeiro.telefone;
    form.email = enfermeiro.email;

  } catch (error) {
    console.error("Erro ao carregar enfermeiro:", error);
    alertType.value = 'error';
    alertMessage.value = 'Não foi possível carregar os dados deste enfermeiro.';
  }
};
...
const salvarEdicao = async () => {
  ...
  try {
    ...
    await EnfermeirosService.atualizar(enfermeiroId, payload);
